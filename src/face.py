...
predictor = dlib.shape_predictor('src/shape_predictor_5_face_landmarks.dat')
face_rec = dlib.face_recognition_model_v1(
    'src/dlib_face_recognition_resnet_model_v1.dat')
    # 打开摄像头
...
def is_real_face(face_image):
    ...
    # 如果第一行和第一列的幅值均小于 35000000，则判断人脸图像是立体的
    if np.max(first_row_magnitude) < 35000000 and np.max(first_column_magnitude) < 35000000:
        return False
    else:
        return True
